...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Automatically kill buffers opened by emacsclient, but only if unmodified
  (setq server-kill-new-buffers t)

  ;; Keep the usual save-confirmation behavior
  (setq server-kill-buffer-query-function 'server-kill-buffer-query-function)

  ;; Ensure we do NOT disable save prompts
  ;; (so don't remove server-kill-buffer-query-function from hooks)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; However, there are too many conflicts with emacs keybindings
  ;; in these states that I end up switching to emacs mode.
  ;; Thus, disabled.
...
* Project
I use built-in Project as opposed to Projectile now.
...
#+BEGIN_SR emacs-lisp
    # (defun my-open-docx-as-org (file)
    #   "Select a DOCX FILE, convert it to Org with pandoc, and open the result."
    #   (interactive "fDOCX file: ")
    #   (let* ((buf-name (format "*docx-org: %s*" (file-name-nondirectory file)))
    ...
    #                             (expand-file-name file) "-t" "org" "--wrap=none"))
    ...
    #         (setq-local my-source-docx file)))
    #     (switch-to-buffer buf)))
    # (defun my-clean-pandoc-org-buffer ()
    # "Normalize pandoc output in the current buffer."
    # (goto-char (point-min))
    # ;; Replace NBSP + normal space with one normal space.
    # (while (search-forward (concat (string #xA0) " ") nil t)
    #     (replace-match " " nil t))
    # ;; Replace any remaining NBSP with normal space.
    # (goto-char (point-min))
    # (while (search-forward (string #xA0) nil t)
    #     (replace-match " " nil t)))

(defun my-replace-nbsp ()
    "Replace non-breaking spaces with normal spaces in the current buffer."
    (interactive)
    (goto-char (point-min))
    (while (search-forward (string #xA0) nil t)
    (replace-match " " nil t)))

    ; Note that this does not remove all \\, just those followed by blank lines.
(defun my-clean-pandoc-org-buffer ()
    "Normalize pandoc Org output for prose manuscripts."
    (interactive)
    (goto-char (point-min))
    ;; Replace NBSP + normal space with a single normal space.
    (while (search-forward (concat (string #xA0) " ") nil t)
        (replace-match " " nil t))
    ;; Replace remaining NBSP with normal spaces.
    (goto-char (point-min))
    (while (search-forward (string #xA0) nil t)
        (replace-match " " nil t))
    ;; Remove hard breaks that occur just before blank lines.
    (goto-char (point-min))
